...
> "No active applications to follow up on. Apply to some roles first with `/jobhunter-op` and come back when they're aging."
...
- Suggest the user run `/jobhunter-op contacto {company}` to find the right person first
...
> - Trying a different contact via `/jobhunter-op contacto`
...
**To:** {email or "No contact found — run `/jobhunter-op contacto` first"}
